...
const ManageUsers = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.error("Neuspješno brisanje korisnika", error)
    }
          ...
          <div className="rounded-t mb-0 px-4 py-3 border-0">
            <div className="flex flex-wrap items-center">
              <div className="relative w-full px-4 max-w-full flex-grow flex-1">
                <h3 className="font-semibold text-base text-blueGray-700">Svi korisnici</h3>
              </div>
          ...
          <table className="items-center bg-transparent w-full border-collapse ">
            <thead>
              <tr>
                ...
                <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">E-mail</th>
                <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Uloga korisnika</th>
            ...
            <tbody>
              {
                data?.users && data?.users.map((user, index) => (
                  <tr key={index}>
                    ...
                    <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left text-blueGray-700 ">{user?.email}</th>
                    <td className="border-t-0 px-6 align-center border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      <span className={`inline-block rounded-full py-[2px] px-3 w-[60px] text-center ${user?.role === "admin" ? "bg-[#1E73BE] text-white" : "bg-amber-300"}`}>
                        {user?.role}
                      </span>
                    </td>
                    <td className="border-t-0 px-6 align-center border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                      ...
                      className="flex gap-1 items-center justify-center"><MdModeEdit />Edit</button>
                    </td>
